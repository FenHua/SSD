...
tf.app.flags.DEFINE_string(
    ...
    'automatically based on whether TensorFlow was built for CPU or GPU.')  # CPU或者GPU下的数据格式
tf.app.flags.DEFINE_float(
    'select_threshold', 0.2, 'Class-specific confidence score threshold for selecting a box.')  # 类别置信度
tf.app.flags.DEFINE_float(
    'min_size', 0.03, 'The min size of bboxes to keep.')  # 最小的bbox
tf.app.flags.DEFINE_float(
    'nms_threshold', 0.45, 'Matching threshold in NMS algorithm.')  # IOU阈值
tf.app.flags.DEFINE_integer(
    'nms_topk', 20, 'Number of total object to keep after NMS.')  # NMS后保留的top-K
tf.app.flags.DEFINE_integer(
    'keep_topk', 200, 'Number of total object to keep for each image before nms.')  # 在NMS前每幅图片保留的bbox
# 检查点相关的配置
tf.app.flags.DEFINE_string(
    'checkpoint_path', '/home/yhq/Desktop/SSD-short/logs',
    'The path to a checkpoint from which to fine-tune.')  # 检查点路经
tf.app.flags.DEFINE_string(
    'model_scope', 'ssd300',
    'Model scope name used to replace the name_scope in checkpoint.')  # 检查点文件的作用范围
...
def main(_):
    with tf.Graph().as_default():
        ...
        all_anchors, all_num_anchors_depth, all_num_anchors_spatial = anchor_creator.get_all_anchors()  # 所有anchor信息
        anchor_encoder_decoder = anchor_manipulator.AnchorEncoder(allowed_borders = [1.0] * 6,
                                                            positive_threshold = None,
                                                            ignore_threshold = None,
                                                            prior_scaling=[0.1, 0.1, 0.2, 0.2])  # 编解码
        decode_fn = lambda pred : anchor_encoder_decoder.ext_decode_all_anchors(pred, all_anchors, all_num_anchors_depth, all_num_anchors_spatial)  # 解码
        with tf.variable_scope(FLAGS.model_scope, default_name=None, values=[features], reuse=tf.AUTO_REUSE):
            ...
            location_pred, cls_pred = ssd_net.multibox_head(feature_layers, FLAGS.num_classes, all_num_anchors_depth, data_format=FLAGS.data_format)  # 预测
        ...
        with tf.device('/cpu:0'):
            ...
            selected_bboxes, selected_scores = parse_by_class(cls_pred, bboxes_pred,
                                                            ...
                                                            FLAGS.keep_topk, FLAGS.nms_topk, FLAGS.nms_threshold)  # 选取最终的bboxes
            labels_list = []
            scores_list = []
            bboxes_list = []
        ...
        with tf.Session() as sess:
            ...
            np_image = imread('/home/yhq/Desktop/SSD-short/demo/test.jpg')
            ...
            imsave('/home/yhq/Desktop/SSD-short/demo/test_out.jpg', img_to_draw)
