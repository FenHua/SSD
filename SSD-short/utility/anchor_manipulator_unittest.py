...
def save_image_with_bbox(image, labels_, scores_, bboxes_):
    ...
    imsave(os.path.join('/home/yhq/Desktop/SSD-short/dataset/debug/{}.jpg').format(save_image_with_bbox.counter), img_to_draw)
    return save_image_with_bbox.counter

def slim_get_split(file_pattern='{}_????'):
    # 在VOC_TFRecords格式中的特征
    keys_to_features = {
    ...
    [org_image, shape, glabels_raw, gbboxes_raw, isdifficult] = provider.get(['image', 'shape','object/label','object/bbox','object/difficult'])
...
if __name__ == '__main__':
    save_image_op = slim_get_split('/home/yhq/Desktop/SSD-short/dataset/tfrecords/train*')
    # 创建图等
    init_op = tf.group([tf.local_variables_initializer(), tf.local_variables_initializer(), tf.tables_initializer()])
    sess = tf.Session()  # 创建一个session用来运行图上的一系列操作
    sess.run(init_op)  # 初始化变量
    coord = tf.train.Coordinator()
    threads = tf.train.start_queue_runners(sess=sess, coord=coord)  # 开始输入队列线程.
    try:
