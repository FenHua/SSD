        ...
        if gt_max_first:
            ...
            left_gt_to_anchors_mask = tf.cast(tf.logical_and(tf.reduce_max(anchors_to_gt_mask, axis=1, keep_dims=True) < 1,
                                                            tf.one_hot(gt_to_anchors, tf.shape(overlap_matrix)[1],
                                                                        on_value=True, off_value=False, axis=1, dtype=tf.bool)), tf.int64)
...
class AnchorEncoder(object):
    ...
    def encode_all_anchors(self, labels, bboxes, all_anchors, all_num_anchors_depth, all_num_anchors_spatial, debug=False):
        # 根据原始图片的大小，y, x, h, w 尺度大小规范为[0, 1]
        # 形状信息: y_on_image, x_on_image: 当前layers_shapes[0] * layers_shapes[1]; h_on_image, w_on_image: anchor的数量
        ...
        with tf.name_scope('encode_all_anchors'):
            ...
            matched_gt_mask = matched_gt > -1  # 得到所有正样本位置
            matched_indices = tf.clip_by_value(matched_gt, 0, tf.int64.max)
            gt_labels = tf.gather(labels, matched_indices)  # 标签可能在非正样本位置混乱
            gt_labels = gt_labels * tf.cast(matched_gt_mask, tf.int64)  # 过滤掉无用的label
            gt_labels = gt_labels + (-1 * tf.cast(matched_gt < -1, tf.int64))  # 将忽略掉的位置设置为 -1
            gt_ymin, gt_xmin, gt_ymax, gt_xmax = tf.unstack(tf.gather(bboxes, matched_indices), 4, axis=-1)
            gt_cy, gt_cx, gt_h, gt_w = self.point2center(gt_ymin, gt_xmin, gt_ymax, gt_xmax)  # 转换到中心点长宽表示格式
            anchor_cy, anchor_cx, anchor_h, anchor_w = self.point2center(anchors_ymin, anchors_xmin, anchors_ymax, anchors_xmax)
            # 编码特征
            # 先验尺度用于平衡回归中loss函数的长宽中心点损伤
            ...
            # 现在gt_localizations是我们回归的目标,在非正样本位置依然存在混乱的情况
            if debug:
    ...
    def decode_all_anchors(self, pred_location, num_anchors_per_layer):
        # 解码过程，函数返回一个list，形状为: [feature_h, feature_w, num_anchors, 4]，顺序为: ymin, xmin, ymax, xmax
...
class AnchorCreator(object):
    def __init__(self, img_shape, layers_shapes, anchor_scales, extra_anchor_scales, anchor_ratios, layer_steps):
        ...
        self._img_shape = img_shape  # 图片大小
        self._layers_shapes = layers_shapes  # 每个特征层大小
        self._anchor_scales = anchor_scales  # 联合anchor_ratio给出长宽
        self._extra_anchor_scales = extra_anchor_scales  # 固定的长宽anchor
        self._anchor_ratios = anchor_ratios # anchor尺寸
        self._layer_steps = layer_steps  # 原始图片与特征图大小比
        self._anchor_offset = [0.5] * len(self._layers_shapes)  # anchor中心点的坐标
    ...
    def get_layer_anchors(self, layer_shape, anchor_scale, extra_anchor_scale, anchor_ratio, layer_step, offset = 0.5):
        ...
        with tf.name_scope('get_layer_anchors'):
            ...
            y_on_image = (tf.cast(y_on_layer, tf.float32) + offset) * layer_step / self._img_shape[0]  # y坐标与原始图片的映射
            x_on_image = (tf.cast(x_on_layer, tf.float32) + offset) * layer_step / self._img_shape[1]  # x坐标与原始图片的映射
            num_anchors_along_depth = len(anchor_scale) * len(anchor_ratio) + len(extra_anchor_scale)  # anchor的深度（一个点几个anchor）
            num_anchors_along_spatial = layer_shape[1] * layer_shape[0]  # anchor放置点的数量
            list_h_on_image = []
            list_w_on_image = []
            global_index = 0
            #--------------------长宽----------------------
    ...
    def get_all_anchors(self):
        all_anchors = []  # anchor信息，中心点坐标，长宽
        all_num_anchors_depth = []  # anchor的深度（一个点几个anchor）
        all_num_anchors_spatial = []  # anchor放置点的数量
