...
def apply_with_random_selector(x, func, num_cases):
  # 计算函数func(x, sel)其中sel属于随机值大小在[0...num_cases-1].
...
def ssd_random_sample_patch(image, labels, bboxes, ratio_list=[0.1, 0.3, 0.5, 0.7, 0.9, 1.], name=None):
  ...
  def check_roi_center(width, height, labels, bboxes):
    ...
    with tf.name_scope('check_roi_center'):
      ...
      def body(index, roi, mask):
        sampled_width, sampled_height = sample_width_height(float_width, float_height)  # 采样的长宽
        x = tf.random_uniform([], minval=0, maxval=width - sampled_width, dtype=tf.int32)  # x位置
        y = tf.random_uniform([], minval=0, maxval=height - sampled_height, dtype=tf.int32)  # y位置
        roi = [tf.cast(y, tf.float32) / float_height,
              tf.cast(x, tf.float32) / float_width,
              tf.cast(y + sampled_height, tf.float32) / float_height,
              tf.cast(x + sampled_width, tf.float32) / float_width]  # ROI区域坐标
        ...
        mask = tf.logical_and(mask_min, mask_max)  # 判断中心点是否在roi内
  ...
  def sample_patch(image, labels, bboxes, min_iou):
    with tf.name_scope('sample_patch'):
      height, width, depth = _ImageDimensions(image, rank=3)  # 图片大小
...
def ssd_random_expand(image, bboxes, ratio=2., name=None):
  ...
  with tf.name_scope('ssd_random_expand'):
    image = tf.convert_to_tensor(image, name='image')  # tensor
    if image.get_shape().ndims != 3:
      raise ValueError('\'image\' must have 3 dimensions.')
    height, width, depth = _ImageDimensions(image, rank=3)
    float_height, float_width = tf.to_float(height), tf.to_float(width)  # float型
    canvas_width, canvas_height = tf.to_int32(float_width * ratio), tf.to_int32(float_height * ratio)  # 帆布大小
    mean_color_of_image = [_R_MEAN/255., _G_MEAN/255., _B_MEAN/255.]  # 平均颜色值
    ...
    scale = tf.cast(tf.stack([height, width, height, width]), bboxes.dtype)  # cast() 用于数据类型的转换
...
def preprocess_for_train(image, labels, bboxes, out_shape, data_format='channels_first', scope='ssd_preprocessing_train', output_rgb=True):
  """
    ...
    输入:
      image: A 任意大小的图片
      labels: A 当前图片中所有boxes的label
      bboxes: A boxes的坐标信息
      out_shape: 预处理后输出的图片大小
      data_format: 输出的数据格式(CPU或GPU)
    """
  with tf.name_scope(scope, 'ssd_preprocessing_train', [image, labels, bboxes]):
    ...
    if orig_dtype != tf.float32:
      image = tf.image.convert_image_dtype(image, dtype=tf.float32)  # 转换到float型范围在[0, 1].
    distort_image = apply_with_random_selector(image,
                                          lambda x, ordering: distort_color(x, ordering, True),
                                          num_cases=4)  # 随机打乱颜色(有4种方法)
    random_sample_image, labels, bboxes = ssd_random_sample_patch_wrapper(distort_image, labels, bboxes)  # ？？？？？？？？？？？？
    random_sample_flip_image, bboxes = random_flip_left_right(random_sample_image, bboxes)  # 随机水平方向翻转
    # 重置大小
    ...
    final_image = _mean_image_subtraction(final_image, [_R_MEAN, _G_MEAN, _B_MEAN])  # 减均值操作
