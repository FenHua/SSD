...
def ssd_multibox_layer(x,num_classes,sizes,ratios,normalization=-1,scope='multibox'):
    '''
    multibox layer:从由Conv4_3，Conv7，Conv8_2，Conv9_2，Conv10_2，Conv11_2特征图经过卷积
    得到的最后detection layer得获取边界框的类别classes、位置location的预测值。
    '''
    pre_shape = x.get_shape().as_list()[1:-1]  # 去除第一个和最后一个得到shape(特征图大小)
    pre_shape = [-1] + pre_shape  # 添加一维[-1,x,x]
    with tf.variable_scope(scope):
        # l2 norm
        if normalization > 0:
            ...
            # print(x)
        # anchors数量
        n_anchors = len(sizes) + len(ratios)  # anchor_size和anchor_ratios
        # locations位置预测值
